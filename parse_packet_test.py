...
_TEST_IPV4_PACKETS = (
    ...
    _TestPacket(raw=bytes.fromhex(
                    ...
                    '08 00 45 00 00 30 3b c3 00 b9 ff 01 37 8c 83 97'
                    '20 81 83 97 20 15 c0 c1 c2 c3 c4 c5 c6 c7 c8 c9'
                    'ca cb cc cd ce cf d0 d1 d2 d3 d4 d5 d6 d7 d8 d9'
                    'da db'
                    ),
                ip_source=IPv4.from_string('131.151.32.129'),  # '83 97 20 81'
    ...
    _TestPacket(raw=bytes.fromhex(
                    ...
                    'dd bf c0 55 01 16 c0 55 01 0e 00 00 00 00 00 00'
                    ),
                ip_source=IPv4.from_string('192.85.1.22'),  # 'c0 55 01 16'
                ip_destination=IPv4.from_string('192.85.1.14')),  # 'c0 55 01 0e'
    )
...
class TestPacketParsing:
    @pytest.mark.parametrize('packet,_1,_2', [
        _TEST_IPV4_PACKETS
        ])
    def test_that_calling_packet_parse_with_an_ipv4_packet_does_not_raise_an_exception(
        ...
        assert_that(calling(parse_packet).with_args(packet), not_(raises(Exception)))
    ...
    @pytest.mark.parametrize('packet,ip_source,ip_destination', [
        _TEST_IPV4_PACKETS
        ])
    def test_that_calling_packet_parse_with_an_ipv4_packet_returns_an_expected_results(
        ...
        assert_that(parse_packet(packet, filter_internal_communication=False, internal_as_source=False),
